...
void adjust_light_level(float *level, int start, int stop,
						float *adjust, float factor)
{
	...
	for (int s = 0; s < SEGMENTS; s++) {
		int x1 = start + width * s / SEGMENTS;
		int x2 = start + width * (s + 1) / SEGMENTS;
...
void adjust_light_levels(Simulator *sim, float factor)
{
	...
	adjust_light_level(sim->black, sim->start, sim->stop,
					   adjust_black, factor);
	adjust_light_level(sim->white, sim->start, sim->stop,
					   adjust_white, factor);
}
