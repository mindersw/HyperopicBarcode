				   ...
				   int *trace, IplImage *debug)
{
	...
	//        p.x, p.y, y_step, dark);
	// CvScalar color = dark ? cvScalar(128, 128, 255, 255) : cvScalar(200, 200, 0, 255);
	int l = 0;
	int r = input->width - 1;
					...
					Peak *peaks, int *traces, IplImage *debug)
{
	...
	while (a) {
		...
		trace[peak.y] = peak.x;
		peak_to_trace(input, peak, count % 2, -1, trace, debug);
		peak_to_trace(input, peak, count % 2, 1, trace, debug);
		count++;
		a = a->next;
	}
				  ...
				  int max_bars, Bar *bars, IplImage *debug)
{
	...
	Peak peaks[size];
	int peaks_count = find_peaks(input, threshold, pt1, pt2, peaks);
	if (peaks_count < 6)
		return 0;
	// printf("found=%d\n", peaks_count);
	peaks_count = best_peaks(peaks, max_bars, debug);
	// printf("reduced=%d\n", peaks_count);

	int traces[peaks_count * input->height];
	int traces_count =
