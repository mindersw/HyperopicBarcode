...
bool measure_quiet_zone(IplImage *input, IplImage *threshold,
					 ...
					 QuietZone *quietzones, IplImage *debug)
{
	...
	if (quietzones[found - 1].width < input->width / 20)
		quietzones[found - 1].width = input->width / 20;
	return found;
}


bool found_two_more_bars(int bars_count, Bar *bars, int first, int last,
		...
		for (int l = 0; l < left_count; l++) {
			for (int r = 0; r < right_count; r++) {
				...
				if (quality > best_quality) {
					...
					first = left_quietzones[l].index;
					last = right_quietzones[r].index;
